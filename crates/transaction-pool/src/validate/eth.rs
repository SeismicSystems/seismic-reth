...
pub struct EthTransactionValidator<Client, T> {
    ...
    /// Whether intrinsic gas includes EIP-2930 access-list charges.
    charge_access_list_gas: bool,
...
impl<Client, Tx> EthTransactionValidator<Client, Tx> {
    ...
    /// Configures whether intrinsic gas includes EIP-2930 access-list charges.
    ///
    /// Enabled by default for Ethereum. Chains whose execution ignores access lists
    /// can disable these charges without changing the signed transaction or other
    /// intrinsic-gas checks.
    pub const fn with_access_list_gas_charging(mut self, enabled: bool) -> Self {
        self.charge_access_list_gas = enabled;
        self
    }
    ...
    Tx: EthPoolTransaction,
{
    ...
    fn validate_one_no_state(
        ...
        if let Err(err) = ensure_intrinsic_gas_with_access_list(
            &transaction,
            &self.fork_tracker,
            self.charge_access_list_gas,
        ) {
            return Err(TransactionValidationOutcome::Invalid(transaction, err))
        }
...
pub fn ensure_intrinsic_gas<T: EthPoolTransaction>(
    ...
    ensure_intrinsic_gas_with_access_list(transaction, fork_tracker, true)
}

/// Checks intrinsic gas with optional EIP-2930 access-list charges.
fn ensure_intrinsic_gas_with_access_list<T: EthPoolTransaction>(
    transaction: &T,
    fork_tracker: &ForkTracker,
    charge_access_list_gas: bool,
) -> Result<(), InvalidPoolTransactionError> {
    ...
    let (access_list_accounts, access_list_storages) = if charge_access_list_gas {
        transaction
            .access_list()
            .map(|list| {
                (list.len(), list.iter().map(|item| item.storage_keys.len()).sum::<usize>())
            })
            .unwrap_or_default()
    } else {
        (0, 0)
    ...
    let gas = revm_interpreter::gas::calculate_initial_tx_gas(
        ...
        access_list_accounts as u64,
        access_list_storages as u64,
...
mod tests {
    use super::*;
    use crate::{
        blobstore::InMemoryBlobStore, error::PoolErrorKind, test_utils::MockTransaction,
    ...
    use alloy_primitives::{hex, Bytes, TxKind, B256, U256};
    ...
    fn intrinsic_gas_without_access_list_preserves_other_costs() {
        let mut create = MockTransaction::eip2930();
        if let MockTransaction::Eip2930 { to, .. } = &mut create {
            *to = TxKind::Create;
        }
        let mut authorization = MockTransaction::eip7702();
        authorization.set_authorization_list(vec![SignedAuthorization::new_unchecked(
            Authorization { chain_id: U256::from(1), address: Address::ZERO, nonce: 0 },
            0,
            U256::from(1),
            U256::from(1),
        )]);
        let calldata = MockTransaction::eip2930().with_input(Bytes::from_static(&[1]));
        for (name, mut tx, prague, required_gas) in [
            ("base transfer", MockTransaction::eip2930(), false, 21_000),
            ("calldata", calldata.clone(), false, 21_016),
            ("calldata floor", calldata, true, 21_040),
            ("creation", create, false, 53_000),
            ("authorization", authorization, true, 46_000),
        ] {
            ...
            tx.set_accesslist(AccessList(vec![AccessListItem {
                address: Address::ZERO,
                storage_keys: vec![B256::ZERO],
            }]));
            tx.set_gas_limit(required_gas - 1);
            assert!(
                matches!(
            ...
            tx.set_gas_limit(required_gas);
            assert!(
                ensure_intrinsic_gas_with_access_list(&tx, &forks, false).is_ok(),
                "must accept exact {name} gas without an access-list surcharge"
            );
            assert!(
                matches!(
            ...
            tx.set_gas_limit(required_gas + 2_400 + 1_900);
