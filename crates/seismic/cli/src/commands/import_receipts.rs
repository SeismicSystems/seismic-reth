...
use reth_seismic_primitives::{bedrock::is_dup_tx, SeismicPrimitives, SeismicReceipt};
...
impl<C: ChainSpecParser<ChainSpec = ChainSpec>> ImportReceiptsOpCommand<C> {
    ...
    pub async fn execute<N: CliNodeTypes<ChainSpec = C::ChainSpec, Primitives = SeismicPrimitives>>(
        self,
    ) -> eyre::Result<()> {
...
where
    N: ProviderNodeTypes<ChainSpec = ChainSpec, Primitives: NodePrimitives<Receipt = SeismicReceipt>>,
    P: AsRef<Path>,
    F: FnMut(u64, &mut Vec<Vec<SeismicReceipt>>) -> usize,
{
...
where
    N: ProviderNodeTypes<Primitives: NodePrimitives<Receipt = SeismicReceipt>>,
    F: FnMut(u64, &mut Vec<Vec<ReceiptTy<N>>>) -> usize,
{
    ...
    while let Some(file_client) =
        reader.next_receipts_chunk::<ReceiptFileClient<OpGethReceiptFileCodec<SeismicReceipt>>>().await?
    {
...
mod test {
    ...
    use reth_chainspec::SEISMIC_MAINNET;
    ...
    async fn filter_out_genesis_block_receipts() {
        ...
        let provider_factory =
            create_test_provider_factory_with_node_types::<SeismicNode>(SEISMIC_MAINNET.clone());
