...
use seismic_alloy_consensus::{SeismicTxType, SeismicTypedTransaction, TxDeposit, TxSeismic};
...
pub struct OvmTransactionSigned {
    ...
    #[deref]
    #[as_ref]
    pub transaction: SeismicTypedTransaction,
}
...
impl OvmTransactionSigned {
    ...
    pub fn decode_rlp_legacy_transaction(data: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        let signed =
            Self { transaction: SeismicTypedTransaction::Legacy(transaction), hash, signature };
        Ok(signed)
    }
}
...
impl Encodable2718 for OvmTransactionSigned {
    ...
    fn encode_2718(&self, out: &mut dyn alloy_rlp::BufMut) {
        match &self.transaction {
            SeismicTypedTransaction::Legacy(tx) => tx.eip2718_encode(&self.signature, out),
            SeismicTypedTransaction::Eip2930(tx) => tx.eip2718_encode(&self.signature, out),
            SeismicTypedTransaction::Eip1559(tx) => tx.eip2718_encode(&self.signature, out),
            SeismicTypedTransaction::Eip7702(tx) => tx.eip2718_encode(&self.signature, out),
            SeismicTypedTransaction::Seismic(tx) => tx.eip2718_encode(&self.signature, out),
        }
    }
}
...
mod tests {
    ...
    fn test_decode_legacy_transactions() {
        ...
        let deposit_tx = match &deposit_decoded.transaction {
            SeismicTypedTransaction::Legacy(ref tx) => tx,
