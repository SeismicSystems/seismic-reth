...
use reth_seismic_node::{node::SeismicNode, utils::test_utils::{
...
// The TestNodeContext can now be simpler, as setup_engine returns NodeTestContext
// which already contains the necessary handles.
// We might just need the client.
pub struct LaunchedSeismicNode {
    pub client: HttpClient,
    _node_test_ctx: NodeHelperType<SeismicNode>,
}

async fn launch_test_seismic_node_e2e() -> LaunchedSeismicNode {
    reth_tracing::init_test_tracing();

    let chain_spec: Arc<ChainSpec> = SEISMIC_MAINNET.clone();

    let attributes_generator = |timestamp: u64| {
        EthPayloadBuilderAttributes::default()
        .into()
    };

    // setup_engine returns a tuple: (Vec<NodeTestContext<...>>, TaskManager, Wallet)
    // We are launching 1 node.
    let (mut nodes, _task_manager, _wallet) = setup_engine::<SeismicNode>(
        1,                     // num_nodes
        chain_spec,            // Arc<SeismicNode::ChainSpec>
        true,                  // is_dev
        attributes_generator,
    )
    .await
    .expect("Failed to setup seismic engine node");

    let node_test_ctx = nodes.remove(0); // Get the first (and only) node

    let rpc_url = node_test_ctx.rpc_url();
    let client = HttpClientBuilder::default()
        .build(rpc_url)
        .expect("Failed to create HTTP client for test node");

    LaunchedSeismicNode {
        client,
        _node_test_ctx: node_test_ctx,
    }
}
...
// In reth-seismic-node/tests/it/integration.rs

#[tokio::test(flavor = "multi_thread")]
async fn test_e2e_simulate_block() { // Renamed for clarity
    let LaunchedSeismicNode { client, _node_test_ctx } = launch_test_seismic_node_e2e().await;

    let chain_id = SEISMIC_MAINNET.chain_id();
    let wallet = Wallet::default().with_chain_id(chain_id);

    let mut nonce = get_nonce(&client, wallet.inner.address()).await;
    println!("Starting nonce for simulate_block: {}", nonce);

    let mut sim_blocks = Vec::new();

    for i in 0..3 { // Create 3 SimBlocks as in the original test
        println!("Preparing SimBlock {}", i + 1);
        // Call 1: Bytes
        let tx_bytes = get_signed_seismic_tx_bytes(
            &wallet.inner,
            nonce,
            TxKind::Create, // Assuming deployment for simulation
            chain_id,
            test_utils::ContractTestContext::get_deploy_input_plaintext(),
        )
        .await;
        ...
        // Ensure SeismicCallRequest can be created from your tx_bytes and tx_typed_data
        // This might involve wrapping them, e.g., if tx_typed_data is not directly Bytes.
        // Assuming get_signed_seismic_tx_typed_data returns something convertible to Bytes or already is.
        // If SeismicCallRequest is an enum { Bytes(Bytes), TypedData(Bytes) /* or specific struct */ }
        // you need to ensure the types match.
        // For now, assuming get_signed_seismic_tx_typed_data returns something that can .into() Bytes for simplicity.
        // If tx_typed_data is a struct, you'd need to serialize it to Bytes or adjust SeismicCallRequest.
        //
        // From your `EthApiOverrideClient<Block>::simulate_v1` call, it implies
        // `SimulatePayload` has `calls: Vec<SeismicCallRequest>`.
        // Let's assume `SeismicCallRequest` has variants like `Bytes(Bytes)` and `TypedData(YourTypedDataStruct)`.
        // The original test used `SeismicCallRequest::Bytes(tx_bytes)` and `SeismicCallRequest::TypedData(tx_typed_data)`.
        // This implies `tx_typed_data` is already the correct type for the `TypedData` variant.

        let block_calls = vec![
            SeismicCallRequest::Bytes(tx_bytes),
            // Assuming get_signed_seismic_tx_typed_data returns the correct struct/enum for TypedData variant
        ...
        println!("SimBlock {} prepared with 2 calls. Next nonce: {}", i + 1, nonce);
    }

    let simulate_payload = SimulatePayload {
        block_state_calls: sim_blocks,
        trace_transfers: false, // As per original
        validation: false,      // As per original
        return_full_transactions: false, // As per original
    };

    println!("Calling eth_simulateV1...");
    // The generic Block type here is alloy_rpc_types::Block
    let simulation_results = EthApiOverrideClient::<Block>::simulate_v1(
        &client,
        simulate_payload,
        None, // block_tag for context (e.g., "latest")
    )
    .await
    .expect("eth_simulateV1 call failed");

    println!("Simulation successful, received {} block results.", simulation_results.len());
    assert_eq!(simulation_results.len(), 3, "Expected 3 block results from simulation");

    for (idx, block_result) in simulation_results.into_iter().enumerate() {
        println!("Processing results for simulated block {}", idx + 1);
        assert!(!block_result.calls.is_empty(), "Simulated block {} has no call results", idx + 1);
        for (call_idx, call_result) in block_result.calls.into_iter().enumerate() {
            // Assuming call_result.return_data is the encrypted bytes
            println!("  Call {} result: status={:?}, gas_used={}, return_data_len={}",
                call_idx + 1,
                call_result.status_code, // alloy_rpc_types::CallResult uses status_code
                call_result.gas_used,
                call_result.return_data.as_ref().map_or(0, |d| d.len())
            );
            assert_eq!(call_result.status_code, Some(1.into()), "Simulated call failed"); // Check for success

            let decrypted_output = client_decrypt(&call_result.return_data.expect("Call result missing return_data"));
            assert_eq!(decrypted_output, test_utils::ContractTestContext::get_code(), "Decrypted output did not match expected contract code");
            println!("  Call {} decrypted output matches expected contract code.", call_idx + 1);
        }
    }
    println!("All simulate_block assertions passed.");
}
