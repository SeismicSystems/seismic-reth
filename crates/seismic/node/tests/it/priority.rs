...
use reth_seismic_payload_builder::builder::SeismicPayloadTransactions;
...
impl SeismicPayloadTransactions<EthPooledTransaction<SeismicTransactionSigned>> for CustomTxPriority {
    fn best_transactions<Pool>(
        &self,
        pool: Pool,
        attr: reth_transaction_pool::BestTransactionsAttributes,
    ) -> impl PayloadTransactions<Transaction = EthPooledTransaction<SeismicTransactionSigned>>
    where
        Pool: reth_transaction_pool::TransactionPool<Transaction = EthPooledTransaction<SeismicTransactionSigned>>,
    {
        // Block composition:
        ...
        let end_of_block_tx = EthPooledTransaction<SeismicTransactionSigned>::from_pooled(Recovered::new_unchecked(
            seismic_alloy_consensus::EthPooledTransaction<SeismicTransactionSigned>::Eip1559(
...
fn build_components<Node>(
    ...
    Node,
    SeismicPoolBuilder,
    BasicPayloadServiceBuilder<SeismicPayloadBuilder<CustomTxPriority>>,
    ...
    >,
{
    ...
    ComponentsBuilder::default()
        ...
        .network(SeismicNetworkBuilder { disable_txpool_gossip, disable_discovery_v4: !discovery_v4 })
...
async fn test_custom_block_priority_config() {
    ...
    let node_handle = NodeBuilder::new(config.clone())
        .with_database(db)
        .with_types_and_provider::<SeismicNode, BlockchainProvider<_>>()
    ...
    let SeismicTypedTransaction::Eip1559(end_of_block_tx) = end_of_block_tx.transaction() else {
