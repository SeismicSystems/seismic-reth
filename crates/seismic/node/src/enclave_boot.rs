//! Utilities for booting the enclave and fetching purpose keys.
...
///
/// # Panics
/// - If the enclave client cannot be built
/// - If purpose keys cannot be fetched after all retries
...
pub async fn boot_enclave_and_fetch_keys<T>(enclave_args: &T) -> GetPurposeKeysResponse
where
    T: AsRef<EnclaveArgs>,
{
    let enclave_args = enclave_args.as_ref();
    // Boot enclave or start mock server
    if enclave_args.mock_server {
        info!(target: "reth::cli", "Starting mock enclave server");
        let addr = enclave_args.enclave_server_addr;
        let port = enclave_args.enclave_server_port;
    ...
    let enclave_client = HttpClientBuilder::default()
        .build(format!(
            "http://{}:{}",
            enclave_args.enclave_server_addr, enclave_args.enclave_server_port
        ))
    ...
    let mut failures = 0;
    while failures <= enclave_args.retries {
        match enclave_client.get_purpose_keys(0).await {
            ...
            Err(e) => {
                warn!(target: "reth::cli", "Failure to fetch purpose keys {}/{}: {}", failures, enclave_args.retries, e);
                tokio::time::sleep(tokio::time::Duration::from_secs(
                    enclave_args.retry_seconds.into(),
                ))
                .await;
                failures += 1;
            }
