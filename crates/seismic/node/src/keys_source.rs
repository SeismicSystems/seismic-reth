...
///
/// Epoch 0 returns [`well_known_purpose_keys`] — the live keys of every pre-TEE
/// network, asserted in lockstep with sanvil's sample keys below. Epochs > 0 derive
/// deterministically from [`DEV_WELL_KNOWN_ROOT`] with the custodian's HKDF scheme.
...
pub fn well_known_purpose_keys_at(epoch: u64) -> eyre::Result<PurposeKeys> {
    if epoch == 0 {
        return Ok(well_known_purpose_keys());
    }
    let tx_io_sk = derive_dev_tx_io_sk(epoch)?;
    let tx_io_pk = tx_io_sk.public_key(&secp256k1::Secp256k1::new());
    let rng_ikm = derive_dev_purpose_bytes::<64>("rng-precompile", epoch, 0)?;
    Ok(PurposeKeys { tx_io_sk, tx_io_pk, rng_ikm })
}
...
mod tests {
    ...
    fn well_known_at_epoch_zero_is_the_well_known_keys() {
        ...
        let well_known = well_known_purpose_keys();
        assert_eq!(at_zero.tx_io_sk, well_known.tx_io_sk);
        assert_eq!(at_zero.tx_io_pk, well_known.tx_io_pk);
        assert_eq!(at_zero.rng_ikm, well_known.rng_ikm);
    }

    /// Epoch > 0 derivation must be deterministic (all built-in nodes agree),
    /// internally consistent (pk is sk's public key), and distinct per epoch.
    #[test]
    fn well_known_at_later_epochs_is_deterministic_and_distinct() {
        ...
        assert_eq!(one_a.tx_io_sk, one_b.tx_io_sk);
        assert_eq!(one_a.rng_ikm, one_b.rng_ikm);
        assert_eq!(one_a.tx_io_pk, one_a.tx_io_sk.public_key(&secp256k1::Secp256k1::new()));

        let two = well_known_purpose_keys_at(2).expect("epoch 2 derivation");
        assert_ne!(one_a.tx_io_sk, two.tx_io_sk);
        assert_ne!(one_a.rng_ikm, two.rng_ikm);

        let zero = well_known_purpose_keys();
        assert_ne!(one_a.tx_io_sk, zero.tx_io_sk);
