...
use reth_seismic_primitives::transaction::signed::OpTransaction;
use reth_seismic_txpool::{interop::MaybeInteropTransaction, SeismicPooledTx};
...
#[derive(Debug, Clone)]
pub struct SeismicPayloadBuilder<Pool, Client, Evm, Txs = ()> {
    ...
    pub config: SeismicBuilderConfig,
    /// The type responsible for yielding the best transactions for the payload if mempool
    /// transactions are allowed.
    pub best_transactions: Txs,
}

impl<Pool, Client, Evm> SeismicPayloadBuilder<Pool, Client, Evm> {
    ...
    /// Configures the builder with the given [`SeismicBuilderConfig`].
    pub fn with_builder_config(
        pool: Pool,
        client: Client,
        evm_config: Evm,
        config: SeismicBuilderConfig,
    ) -> Self {
        Self {
            pool,
            client,
...
impl<Pool, Client, Evm, Txs> SeismicPayloadBuilder<Pool, Client, Evm, Txs> {
    ...
    pub fn with_transactions<T>(
        self,
        best_transactions: T,
    ) -> SeismicPayloadBuilder<Pool, Client, Evm, T> {
...
impl<Pool, Client, Evm, N, T> SeismicPayloadBuilder<Pool, Client, Evm, T>
where
    Pool: TransactionPool<Transaction: SeismicPooledTx<Consensus = N::SignedTx>>,
    Client: StateProviderFactory + ChainSpecProvider<ChainSpec: EthChainSpec + OpHardforks>,
    N: OpPayloadPrimitives,
    Evm: ConfigureEvm<Primitives = N, NextBlockEnvCtx = EthNextBlockEnvAttributes>,
{
...
/// Implementation of the [`PayloadBuilder`] trait for [`SeismicPayloadBuilder`].
impl<Pool, Client, Evm, N, Txs> PayloadBuilder for SeismicPayloadBuilder<Pool, Client, Evm, Txs>
where
...
impl<Txs> OpBuilder<'_, Txs> {
    ...
    where
        EvmConfig: ConfigureEvm<Primitives = N, NextBlockEnvCtx = EthNextBlockEnvAttributes>,
...
pub trait SeismicPayloadTransactions<Transaction>: Clone + Send + Sync + Unpin + 'static {
...
impl<T: PoolTransaction + MaybeInteropTransaction> SeismicPayloadTransactions<T> for () {
...
where
    Evm: ConfigureEvm<Primitives: OpPayloadPrimitives, NextBlockEnvCtx = EthNextBlockEnvAttributes>,
    ChainSpec: EthChainSpec + OpHardforks,
{
