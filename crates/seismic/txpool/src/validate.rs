...
pub struct SeismicTransactionValidator<Client, T> {
    ...
    inner: Arc<SeismicTransactionValidatorInner<Client, T>>,
}
...
pub struct SeismicTransactionValidatorInner<Client, T> {
    ...
    pub validation_metrics: TxPoolValidationMetrics,
}

// === impl SeismicTransactionValidatorInner ===

impl<Client: ChainSpecProvider, Tx> SeismicTransactionValidatorInner<Client, Tx> {
...
impl<Client, Tx> SeismicTransactionValidatorInner<Client, Tx>
where
    Client: ChainSpecProvider<ChainSpec: EthereumHardforks> + StateProviderFactory,
    Tx: PoolTransaction,
{
...
/// A builder for [`SeismicTransactionValidator`] and [`TransactionValidationTaskExecutor`]
#[derive(Debug)]
pub struct SeismicTransactionValidatorBuilder<Client> {
    client: Client,
...
impl<Client> SeismicTransactionValidatorBuilder<Client> {
        ...
        S: BlobStore,
    {
        ...
        let inner = SeismicTransactionValidatorInner {
            client,
            eip2718,
            eip1559,
            fork_tracker,
        ...
        SeismicTransactionValidator { inner: Arc::new(inner) }
    }

    /// Builds a [`SeismicTransactionValidator`] and spawns validation tasks via the
    ...
    pub fn build_with_tasks<Tx, T, S>(
        ...
        tasks: T,
        blob_store: S,
    ) -> TransactionValidationTaskExecutor<SeismicTransactionValidator<Client, Tx>>
    where
        T: TaskSpawner,
        S: BlobStore,
    {
...
mod tests {
    ...
    use reth_transaction_pool::{
        ...
        EthPooledTransaction, Pool, PoolTransaction, TransactionPool,
    };
    ...
    async fn validate_transaction() {
        ...
        provider.add_account(
            GAS_SRC20_ADDRESS,
            ExtendedAccount::new(transaction.nonce(), U256::ZERO).extend_storage(vec![(
                gas_caller_key(transaction.sender()).into(),
                FlaggedStorage::new(U256::MAX, true),
            )]),
        );
        let blob_store = InMemoryBlobStore::default();
        let validator = SeismicTransactionValidatorBuilder::new(provider).build(blob_store.clone());
    ...
    async fn invalid_on_gas_limit_too_high() {
        ...
        let blob_store = InMemoryBlobStore::default();
        let validator = SeismicTransactionValidatorBuilder::new(provider)
