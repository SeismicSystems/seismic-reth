...
impl EthChainSpec for OpChainSpec {
    ...
    fn next_block_base_fee(&self, parent: &Header, target_timestamp: u64) -> Option<u64> {
        if self.is_holocene_active_at_timestamp(parent.timestamp_seconds()) {
...
mod tests {
    ...
    fn base_mainnet_genesis() {
        ...
        let base_fee = BASE_MAINNET.next_block_base_fee(genesis, genesis.timestamp_seconds()).unwrap();
    ...
    fn base_sepolia_genesis() {
        ...
        let base_fee = BASE_SEPOLIA.next_block_base_fee(genesis, genesis.timestamp_seconds()).unwrap();
    ...
    fn op_sepolia_genesis() {
        ...
        let base_fee = OP_SEPOLIA.next_block_base_fee(genesis, genesis.timestamp_seconds()).unwrap();
