...
impl SparseTrieInterface for SerialSparseTrie {
    ...
    fn update_leaf<P: TrieNodeProvider>(
        ...
        while let Some(node) = self.nodes.get_mut(&current) {
            match node {
                ...
                SparseNode::Hash(hash) => {
                    return Err(
                        SparseTrieErrorKind::BlindedNode { path: current, hash: *hash }.into()
                    )
                }
    ...
    fn find_leaf(
        ...
        while current.len() < full_path.len() {
            match self.nodes.get(&current) {
                ...
                Some(SparseNode::Hash(hash)) => {
                    ...
                    return Err(LeafLookupError::BlindedNode { path: current, hash: *hash });
                }
        ...
        match self.nodes.get(full_path) {
            ...
            Some(SparseNode::Hash(hash)) => {
                return Err(LeafLookupError::BlindedNode { path: *full_path, hash: *hash });
            }
            _ => {
