...
pub async fn backup_dir(source: &PathBuf, destination: &PathBuf) -> Result<(), ProviderError> {
    ...
    let metadata = fs::metadata(source_path).await.map_err(|e| ProviderError::FsPathError(format!("Failed to access source path: {}", e)))?;
    ...
    if metadata.is_dir() {
        if !destination_path.exists() {
            fs::create_dir_all(destination_path)
                .await
                .map_err(|e| ProviderError::FsPathError(format!("Failed to create destination directory: {}", e)))?;
        }
        ...
        while let Some((current_src, current_dst)) = entries_stack.pop() {
            let mut entries = fs::read_dir(&current_src)
                .await
                .map_err(|e| ProviderError::FsPathError(format!("Failed to read directory {}: {}", current_src.display(), e)))?;

            while let Some(entry) = entries.next_entry().await.map_err(|e| ProviderError::FsPathError(format!("Failed to get diredctory entry: {}", e)))? {
                let entry_path = entry.path();
                let entry_name = entry.file_name();
                let dst_path = current_dst.join(&entry_name);
                let entry_metadata = entry.metadata().await.map_err(|e| ProviderError::FsPathError(format!("Failed to get diredctory entry: {}", e)))?;

                if entry_metadata.is_dir() {
                    if !dst_path.exists() {
                        fs::create_dir_all(&dst_path)
                            .await
                            .map_err(|e| ProviderError::FsPathError(format!("Failed to create directory {}: {}", dst_path.display(), e)))?;
                    }
                    entries_stack.push((entry_path, dst_path));
                } else {
                    fs::copy(&entry_path, &dst_path)
                        .await
                        .map_err(|e| ProviderError::FsPathError(format!("Failed to copy file from {} to {}: {}", entry_path.display(), dst_path.display(), e)))?;
                }
            }
        }
    } else {
        ...
        if let Some(parent) = destination_path.parent() {
            if !parent.exists() {
                fs::create_dir_all(parent)
                    .await
                    .map_err(|e| ProviderError::FsPathError(format!("Failed to create parent directory {}: {}", parent.display(), e)))?;
            }
        }
        fs::copy(source_path, destination_path)
            .await
            .map_err(|e| ProviderError::FsPathError(format!("Failed to copy file from {} to {}: {}", source_path.display(), destination_path.display(), e)))?;
    }
...
mod tests {
    ...
    async fn create_sample_directory_structure(root: &Path) -> Result<(), ProviderError> {
        ...
        let mut sub_file = File::create(sub_dir.join("file2.txt")).await.expect("crate file failed");
